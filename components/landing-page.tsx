  ...
  return (
    <main className="min-h-screen overflow-hidden bg-[#f7f9fc] text-[#101b33]">
      <header className="absolute inset-x-0 top-0 z-30">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 sm:px-8 sm:py-6 lg:px-10">
          <a href="#top" className="max-w-[70%] font-heading text-xs font-extrabold uppercase tracking-[.1em] text-[#10234a] sm:text-base" aria-label="Ngumu's Eye home">SOCIAL WELFARE &amp; COMMUNITY APP</a>
          ...
          <div className="flex items-center gap-2">
            <a href={apkUrl} target="_blank" rel="noreferrer" className="hidden rounded-full bg-[#078fe8] px-5 py-2.5 text-sm font-bold text-white shadow-lg shadow-[#078fe8]/20 hover:bg-[#0577c5] sm:inline-flex">Get the APK</a>
            <button type="button" className="inline-flex size-11 items-center justify-center rounded-xl border border-[#c7d5e5] bg-white/80 text-[#10234a] md:hidden" aria-expanded={menuOpen} aria-controls="mobile-menu" aria-label={menuOpen ? "Close navigation menu" : "Open navigation menu"} onClick={() => setMenuOpen((open) => !open)}>
              <span className="sr-only">Menu</span><span className="flex flex-col gap-1.5"><span className="h-0.5 w-5 bg-current" /><span className="h-0.5 w-5 bg-current" /><span className="h-0.5 w-5 bg-current" /></span>
            </button>
          </div>
        </div>
        {menuOpen && <nav id="mobile-menu" className="mx-4 rounded-2xl border border-[#c7d5e5] bg-white p-3 shadow-xl md:hidden"><a href="#platform" onClick={() => setMenuOpen(false)} className="block rounded-lg px-4 py-3 font-semibold text-[#60708a] hover:bg-[#eaf5ff]">Platform</a><a href="#plans" onClick={() => setMenuOpen(false)} className="block rounded-lg px-4 py-3 font-semibold text-[#60708a] hover:bg-[#eaf5ff]">Membership</a><a href={apkUrl} target="_blank" rel="noreferrer" className="mt-2 block rounded-lg bg-[#078fe8] px-4 py-3 text-center font-bold text-white">Download the APK</a></nav>}
      </header>

      <section id="top" className="relative bg-[#eaf5ff] pt-32 lg:pt-40"><div className="absolute -right-40 top-0 size-[34rem] rounded-full bg-[#d8edff] blur-3xl" /><div className="mx-auto grid max-w-7xl items-center gap-10 px-5 pb-20 sm:px-8 lg:grid-cols-[1.05fr_.95fr] lg:px-10 lg:pb-28"><div className="relative z-10 max-w-2xl"><div className="mb-8 flex flex-nowrap items-center gap-2 whitespace-nowrap border-b-2 border-[#10234a]/20 pb-5 sm:gap-6 sm:pb-7"><Image src="/ngumus-eye-logo.png" alt="Ngumu's Eye logo" width={180} height={180} priority className="size-16 shrink-0 object-contain sm:size-32 lg:size-40" /><span className="font-heading text-4xl font-extrabold leading-[.98] tracking-[-.055em] sm:text-6xl lg:text-8xl">Ngumu&apos;s Eye</span></div><div className="mb-7 inline-flex items-center gap-2 rounded-full border border-[#b9dcf6] bg-white/70 px-4 py-2 text-xs font-bold uppercase tracking-[.16em] text-[#078fe8]"><span className="size-2 rounded-full bg-[#f2aa35]" /> Community safety, made actionable</div><h1 className="font-heading text-5xl font-extrabold leading-[.98] tracking-[-.055em] sm:text-6xl lg:text-8xl">Look out for <span className="text-[#078fe8]">your people.</span></h1><p className="mt-7 max-w-xl text-lg leading-8 text-[#60708a]">Ngumu&apos;s Eye brings reporting, community awareness, trust, and family support into one calm, practical mobile experience.</p><div className="mt-9 flex flex-col gap-3 sm:flex-row"><a href={apkUrl} target="_blank" rel="noreferrer" className="inline-flex justify-center rounded-xl bg-[#078fe8] px-7 py-4 font-bold text-white shadow-xl shadow-[#078fe8]/25 hover:-translate-y-0.5 hover:bg-[#0577c5]">Download the Android APK <span className="ml-2">↗</span></a><a href="#platform" className="inline-flex justify-center rounded-xl border border-[#c7d5e5] bg-white px-7 py-4 font-bold hover:border-[#078fe8] hover:text-[#078fe8]">See how it works</a></div><div className="mt-6 flex flex-wrap gap-3"><div className="inline-flex items-center gap-3 rounded-xl border border-[#cbd8e7] bg-white px-4 py-3 shadow-sm"><img src="https://cdn.jsdelivr.net/gh/glincker/thesvg@main/public/icons/google-play/default.svg" alt="Google Play" className="size-6 shrink-0" /><div><p className="text-xs font-bold text-[#101b33]">Google Play</p><p className="text-[11px] font-semibold text-[#078fe8]">Coming soon</p></div></div><div className="inline-flex items-center gap-3 rounded-xl border border-[#cbd8e7] bg-white px-4 py-3 shadow-sm"><img src="/apple-logo.svg" alt="Apple App Store" className="size-6" /><div><p className="text-xs font-bold text-[#101b33]">App Store</p><p className="text-[11px] font-semibold text-[#078fe8]">Coming soon</p></div></div></div><div className="mt-8 flex flex-wrap gap-x-6 gap-y-2 text-sm font-semibold text-[#708097]"><span>Android 8.0+</span><span>Free to download</span><span>Made for Namibia</span></div></div><div className="relative flex justify-center lg:justify-end"><div className="absolute bottom-8 h-20 w-72 rounded-full bg-[#078fe8]/20 blur-3xl" /><Image src="/phone-feed.png" alt="Ngumu's Eye community feed screen" width={444} height={700} priority className="relative w-[290px] drop-shadow-[0_35px_40px_rgba(16,35,74,.25)] sm:w-[350px] lg:w-[405px]" /></div></div></section>
